...
js=[
	"${jspath}/sisyphus.min.js"
	]
>
<@we.breadcrumb current="添加多图文"/>
...
<script>
	...
	$(function() {
		...
		$(".Js_material_submit").on("click", function() {
			...
			//	$(this).require("请输入链接").maxLength(100);
			//});
			...
			var posturl = "${base}/admin/project/media/addMultiple", postdata = {
				"titles" : _title,
			...
			WG.fn.ajax(posturl, postdata, function(data) {
				if (data.success == true) {
					window.location.href="${bash}/admin/project/media";
