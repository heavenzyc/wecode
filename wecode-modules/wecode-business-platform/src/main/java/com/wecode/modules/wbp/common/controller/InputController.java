...
public class InputController extends BaseController{
    ...
    public void list(){
        ...
        Page<InputInfo> page = InputInfo.getPage(getParaToInt("page", 1),getParaToInt("rows", 10),transport,accepter,weighter,startStr,endStr);
...
//        List<AcceptMerchant> accepts = AcceptMerchant.getList();
    ...
    public void exportExcel() {
        ...
        for (int i = 0; i < infos.size(); i++) {
            InputInfo info = infos.get(i);
            row = sheet.createRow(i + 1);
            if (info.getDate("input_time") != null) {
                row.createCell(0).setCellValue(DateUtils.format(info.getDate("input_time"), "yyyy/MM/dd"));
            }
        ...
        try {
            ...
            String fileName = "入库单-" + new SimpleDateFormat("MMddHHmmSSS").format(new Date()) + ".xls";
