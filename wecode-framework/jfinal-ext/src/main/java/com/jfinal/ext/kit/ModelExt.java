...
public class ModelExt<M extends ModelExt<M>> extends Model<M> {
	...
	private static List<CallbackListener> callbackListeners = Lists
			.newArrayList();
	...
	public ModelExt() {
		...
		clazz = (Class<? extends ModelExt<M>>) ((ParameterizedType) genericSuperclass)
	...
	public boolean save() {
		...
		if (pseudoDelete()) {
			if (!tableInfo.hasColumnLabel(deleteColumnLabel)) {
				throw new ActiveRecordException("The deleteColumnLabel ("
						+ deleteColumnLabel + ") is not exist");
			}
	...
	public boolean deleteById(Object id) {
		...
		if (pseudoDelete()) {
			...
			String sql = "update " + tableInfo.getName() + " set "
					+ deleteColumnLabel + " = 1 where " + pKey + " = ?";
	...
	public int deleteByColumn(String column, Object value) {
		return deleteByColumns(Lists.newArrayList(column),
	...
	public int deleteByColumns(List<String> columns, List<Object> values) {
		...
		if (pseudoDelete()) {
			...
			String pKey = tableInfo.getPrimaryKey();
			sql += "update " + tableInfo.getName() + " set "
					+ deleteColumnLabel + " = 1";
		} else {
	...
	public List<M> findAll() {
		...
		if (pseudoDelete()) {
			sql += " where " + deleteColumnLabel + " is null or "
					+ deleteColumnLabel + " !=1 ";
		}
		return find(sql);
	}
	...
	public M findFirstByColumn(String column, Object value) {
		List<M> result = findByColumns(Lists.newArrayList(column),
				Lists.newArrayList(value));
		return result.size() > 0 ? result.get(0) : null;
	}

	public List<M> findByColumn(String column, Object value) {
		return findByColumns(Lists.newArrayList(column),
				Lists.newArrayList(value));
	}
	...
	public List<M> findByColumns(List<String> columns, List<Object> values) {
		...
		if (pseudoDelete()) {
			sql += " and " + deleteColumnLabel + " is null or "
					+ deleteColumnLabel + " !=1 ";
		}
		for (String column : columns) {
			sql += " and " + column + " = ?";
	...
	public List<M> children(Class<? extends Model> model) {
		...
		String childTableName = TableMapping.me().getTable(childModel)
				.getName();
		String primaryKey = TableMapping.me().getTable(clazz).getPrimaryKey();
		try {
			return childModel.newInstance().find(
					"select * from " + childTableName + " where " + foreignKey
							+ "= ?", get(primaryKey));
		} catch (Exception e) {
	...
	public M parent(Class<? extends Model> model) {
		...
		String parentTableName = TableMapping.me().getTable(parentModel)
				.getName();
		String primaryKey = TableMapping.me().getTable(clazz).getPrimaryKey();
		try {
			return (M) parentModel.newInstance().findFirst(
					"select * from " + parentTableName + " where " + foreignKey
							+ "= ?", get(primaryKey));
		} catch (Exception e) {
			throw new ActiveRecordException(e.getMessage(), e);
		}
