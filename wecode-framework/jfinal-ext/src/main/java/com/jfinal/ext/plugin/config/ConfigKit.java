...
public class ConfigKit {
	 ...
	 * @param reload
	 */
	static void init(List<String> includeResources,
			List<String> excludeResources, boolean reload) {
...
//				}
				...
				for (String exclude : excludeResources) {
					if (Pattern.compile(exclude).matcher(file.getName())
							.matches()) {
						excluded = true;
					}
				}
				...
				if (fileName.endsWith("-test." + ConfigPlugin.suffix)) {
					try {
						testMap.putAll(ResourceKit
								.readProperties(testFileName(fileName)));
					} catch (Exception e) {
	...
	private static String testFileName(String fileName) {
		return fileName.substring(0,
				fileName.indexOf("." + ConfigPlugin.suffix))
				+ "-test." + ConfigPlugin.suffix;
	}
	...
	public static String getRequiredStr(String key) {
		String str = getStr(key);
		if(StringUtils.isBlank(str)) {
			throw new RuntimeException("请配置" + key);
		}
		return str;
	}
